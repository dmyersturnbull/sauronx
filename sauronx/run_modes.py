...
from .component_check import ComponentChecker
...
class CheckMode(RunMode):
    def run(self, alive_ms: int) -> None:
        output_dir = pjoin(config.output_dir_root, "component-checks", plain_timestamp_started)
        remake_dirs(output_dir)
        with SauronxAlive(None):
            SauronxLock().lock(None)
            try:
                checker = ComponentChecker(output_dir, duration_ms=alive_ms)
                checker.run_all()
            finally:
                SauronxLock().unlock()
